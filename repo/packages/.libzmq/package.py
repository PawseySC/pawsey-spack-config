...
class Libzmq(AutotoolsPackage):
    ...
    def _cray_runtime_rpath_flags(self):
        """Return linker RPATH flags for Cray runtime libraries.

        This avoids hard-coding versioned paths such as:
            /opt/cray/libfabric/2.3.1/lib64

        Instead, it reuses paths already provided by the loaded Cray
        programming environment.
        """

        rpaths = []

        for env_var in ("CRAY_LD_LIBRARY_PATH", "LD_LIBRARY_PATH"):
            value = os.environ.get(env_var, "")
            if value:
                rpaths.extend(value.split(":"))

        # Keep only Cray/XPMEM runtime paths.
        #
        # This intentionally avoids adding arbitrary user paths from
        # LD_LIBRARY_PATH into the installed RPATH.
        rpaths = [
            p
            for p in rpaths
            if p.startswith("/opt/cray/") or p.startswith("/opt/xpmem/")
        ]

        # Remove duplicates while preserving order.
        rpaths = list(dict.fromkeys(rpaths))

        return ["-Wl,-rpath,{0}".format(p) for p in rpaths]
    ...
    def configure_args(self):
        ...
        cray_rpath_flags = self._cray_runtime_rpath_flags()
        if cray_rpath_flags:
            config_args.append("LDFLAGS={0}".format(" ".join(cray_rpath_flags)))
