...
class Libpressio(CMakePackage, CudaPackage):
    ...
    def flag_handler(self, name, flags):
        if name == "cxxflags" and self.spec.satisfies("%cce"):
            flags.extend([
                "-include", "cstdint",
                "-include", "algorithm",
            ])
    ...
    @run_before("cmake")
    def fix_cce_basic_indexer(self):
        if not self.spec.satisfies("%cce"):
            return
    
        path = join_path(self.stage.source_path, "src", "basic_indexer.h")
    
        with open(path, "r") as f:
            contents = f.read()
    
        marker = (
            "template <class T, template <class,  size_t> class array_type, "
            "class V, size_t N>"
        )
    
        if marker not in contents:
            raise RuntimeError(
                "Could not find expected as() declaration in src/basic_indexer.h"
            )
    
        overload = r"""
    template <class T, class V, size_t N>
    basic_indexer<T, N> as(basic_indexer<V, N> const& in) {
      std::array<T, N> arr;
    ...
    """
        ...
        contents = contents.replace(marker, overload + marker, 1)
