...
class Syclgromacs(CMakePackage, CudaPackage, ROCmPackage):
    ...
    conflicts("+sycl", when="+rocm", msg="ROCm support is implemented via hipSYCL.")
    conflicts(
...
class CMakeBuilder(spack.build_systems.cmake.CMakeBuilder):
    ...
    def cmake_args(self):
        options = []
        cc_compiler = spack_cc
        cxx_compiler = spack_cxx
        if "+rocm" in self.spec or "++rocm" in self.spec:
            cc_compiler = self.spec["hip"].prefix.bin.hipcc
            cxx_compiler = self.spec["hip"].prefix.bin.hipcc
            amdgpu_target = ",".join(self.spec.variants["amdgpu_target"].value)
            options.extend(
                    [
                        "-DGMX_GPU=SYCL",
                        "-DGMX_SYCL_HIPSYCL=ON",
                        f"-DHIPSYCL_TARGETS='hip:{amdgpu_target}'"
                    ])
        ...
        if "+mpi" in self.spec:
            ...
            if self.pkg.version < Version("2020"):
                ...
                options.extend(
                    [
                        "-DCMAKE_C_COMPILER=%s" % cc_compiler,
