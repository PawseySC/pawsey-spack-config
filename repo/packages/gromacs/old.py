...
class Gromacs(CMakePackage, CudaPackage, ROCmPackage):
    ...
    variant("hip", default=False, description="Enable GPU compilation with the HIP backend.")
    ...
    def cmake_args(self):
        ...
        if self.spec.satisfies("+sycl"):
            options.extend([
                ...
                options.extend([
                    f"-DCMAKE_HIP_COMPILER={self.spec['hip'].prefix.bin.hipcc}",
                    f"-DCMAKE_PREFIX_PATH={self.spec['hip'].prefix}",
                    "-DGMX_GPU=HIP"])
