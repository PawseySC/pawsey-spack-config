...
# Pawsey, Ilkhom: removed options os.path.basename(self.compiler.cc) and os.path.basename(self.compiler.fc)
# New versions of charmpp does not take an option which indicates compiler.
...
class Charmpp(Package):
    ...
    # Charm++ 8.0.0 compatibility with libfabric 2.x
    patch(
        "charmpp-8.0.0-libfabric2-container-of.patch",
        when="@8.0.0 backend=ofi",
    )

    # ROMIO bundled with Charm++ 8.0.0 uses implicit-int configure tests,
    # which GCC 14 rejects as errors.
    patch(
        "charmpp-8.0.0-romio-gcc14-implicit-int.patch",
        when="@8.0.0 %gcc@14:",
    )

    # Charm++ 6.10.2 compatibility with newer libfabric
    patch(
        "charmpp-6.10.2-libfabric2-compat.patch",
        when="@6.10.2 backend=ofi",
    )

    # Charm++ 6.10.2 bundled ROMIO uses implicit-int configure tests,
    # which GCC 14 rejects as errors.
    patch(
        "charmpp-6.10.2-romio-gcc14-implicit-int.patch",
        when="@6.10.2 %gcc@14:",
    )
