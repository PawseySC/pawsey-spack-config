...
class Adios2(CMakePackage, CudaPackage, ROCmPackage):
    ...
    patch("fix_submodules.patch", when="%cce@19:")
    ...
    def setup_build_environment(self, env: EnvironmentModifications) -> None:
        ...
        if self.spec.satisfies("+python"):
            ...
            for directory in {python_pkg.purelib, python_pkg.platlib}:
                env.prepend_path(
                    "PYTHONPATH",
                    join_path(numpy_prefix, directory),
                )
    ...
    def _numpy_include_dir(self):
        numpy_prefix = str(self.spec["py-numpy"].prefix)
    
        for root, _, files in os.walk(numpy_prefix):
            if os.path.basename(root) == "numpy" and "arrayobject.h" in files:
                return os.path.dirname(root)
    
        raise InstallError(
            "Unable to locate the NumPy include directory under {0}".format(
                numpy_prefix
            )
        )
    ...
    def cmake_args(self):
        ...
        if spec.satisfies("+python"):
            ...
            numpy_include = self._numpy_include_dir()
        
            args.extend(
                [
                    self.define("Python_NumPy_INCLUDE_DIR", numpy_include),
                    self.define("Python3_NumPy_INCLUDE_DIR", numpy_include),
                ]
            )
