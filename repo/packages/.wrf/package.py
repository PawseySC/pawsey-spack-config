...
class Wrf(Package):
    ...
    def answer_configure_question(self, outputbuf):
        ...
        if "Please select from among the following" in outputbuf:
            ...
            print(f"Collected platform options: {options.keys()}")
    
            # Adjust compiler pair logic for Cray platforms
            compiler_map = {
                "ftn": "gfortran",  # Map Cray Fortran to gfortran
                "cc": "gcc",        # Map Cray C to gcc
            }
            comp_pair = "%s/%s" % (
                compiler_map.get(basename(self.compiler.fc).split("-")[0], self.compiler.fc),
                compiler_map.get(basename(self.compiler.cc).split("-")[0], self.compiler.cc),
            )
            print(f"Compiler pair: {comp_pair}")
    
            compiler_matches = dict((x, y) for x, y in options.items() if comp_pair in x.lower())
            print(f"Filtered compiler matches: {compiler_matches}")
    
            if not compiler_matches:
                raise InstallError(
                    f"No valid compiler matches found for {comp_pair}. Available options: {options}"
                )
            ...
            try:
                ...
                raise InstallError(
                    f"build_type {self.spec.variants['build_type'].value} unsupported for compilers {comp_pair}. Options: {compiler_matches}"
                )
...
#        if "Please select from among the following" in outputbuf:
#            options = collect_platform_options(outputbuf)
#            print(f"Collected platform options: {options}")
#            comp_pair = "%s/%s" % (
