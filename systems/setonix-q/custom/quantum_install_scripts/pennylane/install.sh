...
if should_install_software; then
    ...
    python -m pip install --prefix="${prefix_dir}" "pennylane==${tool_ver}" ${build_dir}/pennylane_lightning*.whl || {
        echo "Error: Failed to install PennyLane and Lightning packages"
        exit 1
    }
